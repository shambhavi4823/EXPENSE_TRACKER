...
public class ExpenseDAO {
    ...
    public void addExpense(int userId, String itemType, String itemName, int qty, double price) {
        try (Connection con = DriverManager.getConnection(url, user, password)) {
            ...
            double monthlyLimit = 10000.00; // Fallback default
            ...
            String spentQuery = "SELECT SUM(price) FROM expenses WHERE user_id = ? AND MONTH(date_added) = MONTH(CURRENT_DATE()) AND YEAR(date_added) = YEAR(CURRENT_DATE())";
            ...
            try (PreparedStatement spentStmt = con.prepareStatement(spentQuery)) {
                ...
                try (ResultSet rsSpent = spentStmt.executeQuery()) {
                    if (rsSpent.next()) currentSpent = rsSpent.getDouble(1);
                }
            }

            if ((currentSpent + price) > monthlyLimit) {
                System.out.println("\n❌ ERROR: Monthly Budget Exceeded! Transaction Intercepted.");
                return;
            }
        ...
        try (Connection con = DriverManager.getConnection(url, user, password)) {
            ...
            try (PreparedStatement pstmt = con.prepareStatement(userQuery)) {
                ...
                try (ResultSet rs = pstmt.executeQuery()) {
                    if (rs.next() && rs.getDouble("monthly_budget") > 0) budgetLimit = rs.getDouble("monthly_budget");
                }
            }
            ...
            try (PreparedStatement pstmt = con.prepareStatement(recentQuery)) {
                ...
                try (ResultSet rs = pstmt.executeQuery()) {
                    while (rs.next()) {
                        if (recentExpensesJson.length() > 0) recentExpensesJson.append(",");
    ...
    public void deleteExpense(int userId, int expenseId) {
        ...
        try (Connection con = DriverManager.getConnection(url, user, password);
            ...
            pstmt.setInt(2, userId);
            if (pstmt.executeUpdate() > 0) System.out.println("✅ Expense deleted successfully!");
        } catch (SQLException e) {
    ...
    public void editExpense(int userId, int expenseId, String newType, String newName, int newQty, double newPrice) {
        ...
        try (Connection con = DriverManager.getConnection(url, user, password);
            ...
            if (pstmt.executeUpdate() > 0) System.out.println("✅ Expense updated successfully!");
        } catch (SQLException e) {
            System.out.println("❌ Database Error: " + e.getMessage());
        }
    }
    ...
    public void searchExpenses(int userId, String searchType, String keyword) {
        String query = searchType.equalsIgnoreCase("category") ? 
            "SELECT * FROM expenses WHERE user_id = ? AND item_type LIKE ?" : 
            "SELECT * FROM expenses WHERE user_id = ? AND DATE(date_added) = ?";
        try (Connection con = DriverManager.getConnection(url, user, password);
             PreparedStatement pstmt = con.prepareStatement(query)) {
            pstmt.setInt(1, userId);
    ...
    public void adminDeleteExpense(int expenseId) {
        ...
        try (Connection con = DriverManager.getConnection(url, user, password);
             PreparedStatement pstmt = con.prepareStatement(query)) {
            pstmt.setInt(1, expenseId);
            if (pstmt.executeUpdate() > 0) System.out.println("✅ Success (Admin): Expense ID removed!");
        } catch (SQLException e) {
            System.out.println("❌ Database Error: " + e.getMessage());
        }
    }
        ...
        try (Connection con = DriverManager.getConnection(url, user, password);
            ...
            while (rs.next()) {
                System.out.printf("%-5d | %-15s | %-12s | %-15s | %-5d | ₹%-9.2f | %-15s\n", 
                                  ...
                                  rs.getString("item_name"), rs.getInt("quantity"), rs.getDouble("price"), rs.getString("date_added"));
            }
        } catch (SQLException e) {
            System.out.println("❌ Database Error: " + e.getMessage());
        }
