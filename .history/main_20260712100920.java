...
public class main {
    public static void main(String[] args) {
        try {
            ...
            // Core Gateway Binding Routing Table
            server.createContext("/api/register", new RegisterHandler());
            server.createContext("/api/login", new LoginHandler());
            server.createContext("/api/user-dashboard", new UserDashboardHandler());
            server.createContext("/api/add-expense", new AddExpenseHandler());
            server.createContext("/api/search-expenses", new SearchExpensesHandler()); // NEWLY BOUND SEARCH ROUTE
    ...
    static class AddExpenseHandler implements HttpHandler {
        ...
        public void handle(HttpExchange exchange) throws IOException {
            ...
            if ("POST".equalsIgnoreCase(exchange.getRequestMethod())) {
                ...
                int userId = Integer.parseInt(data.get("userId"));
                String itemType = data.get("item_type");
                String itemName = data.get("item_name");
                int qty = Integer.parseInt(data.get("quantity"));
                double price = Double.parseDouble(data.get("price"));

                String jsonResponse = new ExpenseDAO().addExpense(userId, itemType, itemName, qty, price);
    ...
    // INTERCEPTOR HANDLER FOR SEARCH FILTER REQUESTS
    static class SearchExpensesHandler implements HttpHandler {
        ...
        public void handle(HttpExchange exchange) throws IOException {
            ...
            if ("POST".equalsIgnoreCase(exchange.getRequestMethod())) {
                ...
                // Calls ExpenseDAO method to return JSON array data
                String jsonResponse = new ExpenseDAO().searchExpensesJson(userId, searchType, keyword);
