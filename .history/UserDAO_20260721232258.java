...
public class UserDAO {
    ...
    // --- NEW METHOD FOR ADMIN ANALYTICS DASHBOARD ---
    public String getAdminAnalyticsJson() {
        ...
        String expensesQuery = "SELECT SUM(quantity * price), COUNT(*) FROM expenses";

        try (Connection con = DriverManager.getConnection(url, dbuser, dbPass)) {
            // Get total user count
            try (PreparedStatement pstmt1 = con.prepareStatement(usersQuery);
            ...
            // Get total spent and total expense transactions
            try (PreparedStatement pstmt2 = con.prepareStatement(expensesQuery);
                 ResultSet rs2 = pstmt2.executeQuery()) {
                if (rs2.next()) {
                    globalExpenses = rs2.getDouble(1);
                    totalQueries = rs2.getInt(2);
                }
            }

            return String.format(
                "{\"status\":\"success\", \"totalUsers\":%d, \"globalExpenses\":%.2f, \"totalQueries\":%d}",
                totalUsers, globalExpenses, totalQueries
            );
        } catch (SQLException e) {
            System.out.println("❌ Error fetching Admin Analytics: " + e.getMessage());
